...
\NEW{ItemEnvelope}{\iTruth}{
  ...
  \s\MYnumber	{00001}
  \s\MYtext	{A thick book with gold-edged pages.}
  \bulky	{1}
  \s\Myabils	{\aTruth{}\aRealityWarp{}}
  }

\NEW{ItemEnvelope}{\iDeathOne}{
  \s\MYname	{The Scythe of Death}
  \s\MYnumber	{00003}
  \s\MYtext	{A wickedly sharp scythe with a terrifying aura.}
  \bulky	{1}
  \s\Myabils	{\aWither{}\aDishearten{}}
  }
  
\NEW{ItemEnvelope}{\iDeathTwo}{
  \s\MYname	{The Scythe of Death}
  \s\MYnumber	{00003}
  \s\MYtext	{A wickedly sharp scythe with a terrifying aura.}
  \bulky	{1}
  \s\Myabils	{\aWither{}\aDishearten{}\aDeathGlare{}}
  }

\NEW{ItemEnvelope}{\iWar}{
  \s\MYname	{The Axe of War}
  \s\MYnumber	{00004}
  \s\MYtext	{A terrifyingly large axe that is strangely easy to wield.}
  \bulky	{1}
  \s\Myabils	{\aBerzerker{}\aRageGaze{}}
  }

\NEW{ItemEnvelope}{\iFamineOne}{
  \s\MYname	{The Basket of Famine}
  \s\MYnumber	{00005}
  \s\MYtext	{An empty basket that emanates desperation.}
  \bulky	{1}
  \s\Myabils	{\aWither{}}
  }
  
\NEW{ItemEnvelope}{\iFamineTwo}{
  \s\MYname	{The Basket of Famine}
  \s\MYnumber	{00005}
  \s\MYtext	{An empty basket that emanates desperation.}
  \bulky	{1}
  \s\Myabils	{\aWither{}\aJusticeHunger{}}
  }
  
\NEW{ItemEnvelope}{\iFamineThree}{
  \s\MYname	{The Basket of Famine}
  \s\MYnumber	{00005}
  \s\MYtext	{An empty basket that emanates desperation.}
  \bulky	{1}
  \s\Myabils	{\aWither{}\aFleshHunger{}}
  }

\NEW{ItemEnvelope}{\iPestilenceOne}{
  \s\MYname	{The Mask of Pestilence}
  \s\MYnumber	{00006}
  \s\MYtext	{A plague doctor mask.}
  \bulky	{1}
  \s\Myabils	{\aWither{}\aDishearten{}\aImmunity{}}
  }
  
\NEW{ItemEnvelope}{\iPestilenceTwo}{
  \s\MYname	{The Mask of Pestilence}
  \s\MYnumber	{00006}
  \s\MYtext	{A plague doctor mask.}
  \bulky	{1}
  \s\Myabils	{\aWither{}\aDishearten{}\aImmunity{}\aPlague{}}
  }

\NEW{ItemEnvelope}{\iLove}{
  \s\MYname	{The Pendant of Love}
  \s\MYnumber	{00007}
  \s\MYtext	{A softly glowing pendant in the shape of a heart on a golden chain.}
  \bulky	{1}
  \s\Myabils	{\aHeartEyes{}\aHearten{}}
  }
  
\NEW{ItemEnvelope}{\iBeauty}{
  \s\MYname	{The Mirror of Beauty}
  \s\MYnumber	{00011}
  \s\MYtext	{A silver mirror with an etching of Silverside at dawn.}
  \bulky	{1}
  \s\Myabils	{\aIllusion{}\aShiny{}}
  }
  
\NEW{ItemEnvelope}{\iHonor}{
  \s\MYname	{The Shield of Honor}
  \s\MYnumber	{00015}
  \s\MYtext	{A shield with images of home and family engraved into the surface.}
  \bulky	{1}
  \s\Myabils	{\aAppeal{}\aRousingSpeech{}\aGuiltTrip{}}
  }
  
\NEW{ItemEnvelope}{\iIngenuity}{
  \s\MYname	{The Multitool of Ingenuity}
  \s\MYnumber	{00013}
  \s\MYtext	{A complex multitool.}
  \bulky	{1}
  \s\Myabils	{\aFix{}\aTech{}}
  }

\NEW{ItemEnvelope}{\iLoyalty}{
  \s\MYname	{The Sword Loyalty}
  \s\MYnumber	{00017}
  \s\MYtext	{An elaborate sword glowing with golden light.}
  \bulky	{1}
  \s\Myabils	{}
  }
